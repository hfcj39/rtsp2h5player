...
#include <memory.h>
#define BUFSIZE 4096
unsigned long FLVClient::TickCount(){
#ifdef WIN32
	return GetTickCount();
#else
	struct timespec ts;
...
uint8_t * FLVClient::get_nal(uint32_t *len, uint8_t **offset, uint8_t *start, uint32_t total)
{
	uint32_t info;
	uint8_t *q;
	uint8_t *p = *offset;
...
int FLVClient::DataCallBack(unsigned char* data, int len, unsigned int timestamp)
{
	if (_basictick == ~0){
		_basictick = TickCount();
	}
	if (timestamp == ~0){
		timestamp = TickCount() - _basictick;
	}
	...
	if (isfirst){
		...
		if (SendMessage(flv_header, 13) < 0){
			return -1;
		}
		isfirst = false;
	}
...
int FLVClient::writespspps(uint8_t * sps, uint32_t spslen, uint8_t * pps, uint32_t ppslen, uint32_t timestamp)
{
	...
	if (SendMessage(output, output_len) < 0){
		//RTMP Send out
		free(output);
		return -1;
	}
...
int FLVClient::writeavcframe(uint8_t * nal, uint32_t nal_len, uint32_t timestamp, bool IsIframe)
{
...
int FLVClient::SendMessage(uint8_t * output, int output_len)
{
#ifdef _DEBUG
	int _index = 0;
	int _ret = 0;
